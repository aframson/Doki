...
export const GetTripDistance = async (platform, data) => {
    if (data) {
        ...
        try{
            if (arr.length == 0 || arr.length == 1) {
                ...
                let res = await getDriveTime(platform, startLoc, destLoc);
                ...
                let res = await getDriveTime(platform, origins, arr[arr.length - 1].lat + ',' + arr[arr.length - 1].lng);
                distance = res.distance_in_km;
            }
        }catch(error){
            console.log(error);
