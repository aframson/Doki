            ...
            <View style={styles.statusStyle}>
              ...
              <View style={styles.statusView}>
                {item.type && item.type == "Credit"
                  ? setCredits(item.amount)
                  : null}
                {item.type && item.type == "Debit"
                  ? setDebits(item.amount)
                  : null}
                {item.type && item.type == "Credit" ? (
                  <Text style={styles.historyamounttextStyle}>
                    {language.credited +
                      " " +
                      settings.symbol +
                ...
                {item.type && item.type == "Debit" ? (
                  <Text style={styles.historyamounttextStyle}>
                    {language.debited +
                      " " +
                      settings.symbol +
                      parseFloat(debited - credited).toFixed(0)}
                  </Text>
                ) : null}
                ...
                <Text style={styles.textStyle}>
                  {language.Transaction_Id} {item.txRef}
                </Text>
